...
//*************************发送/接收数据缓存区******************** 
char sendBuff[COM_BUF_SIZE] = {0};
char recBuff[COM_BUF_SIZE] = {0};
//**********************发送/接收数据缓存区 结束******************
...
void* serial_test(void * args)
{
    PORT COM1;
    int rcv_len = 0;

    COM1 = serial_init(COM_PORT, COM_BAUD, COM_BITS, COM_STOPBITS, COM_PARITY);
    
    memset(sendBuff, 0, COM_BUF_SIZE);							//清空发送缓存区 
    memset(recBuff, 0, COM_BUF_SIZE);							//清空接收缓存区
    
    while(1)
    {
        rcv_len = Serial_ReciveData(COM1, recBuff, COM_BUF_SIZE);
        road.pRecData(&car);
        Sleep(COM_TIME);
        
        Serial_SendData(COM1, sendBuff, COM_BUF_SIZE); 
        Sleep(COM_TIME);
    }
}
